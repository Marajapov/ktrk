<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Authentication Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines are used during authentication for various
    | messages that we need to display to the user. You are free to modify
    | these language lines according to your application's requirements.
    |
    */

    // Nav
    'NavAbout'  =>  'О КТРК',
    'NavAboutHistory'  =>  'История',
    'NavAboutAdministration'  =>  'Руководство',
    'NavAboutNS'  =>  'Наблюдательный совет',
    'NavAboutRS'  =>  'Редакционный советник',
    'NavAboutStrategy'  =>  'Стратегия',
    'NavAboutReport'  =>  'Отчеты',
    'NavAboutBase'  =>  'Нормативная база',
    'NavAboutBranding'  =>  'Брендинг',
    'NavAboutRTC'  =>  'Республиканский телецентр',

    'NavNews'  =>  'Новости',

    'NavTeleprogram'  =>  'Программа передач',
    'NavTeleprogramKtrk'  =>  'Общественный первый Канал',
    'NavTeleprogramMusic'  =>  'Телеканал Музыка',
    'NavTeleprogramMadaniyat'  =>  'Телеканал Маданият Тарых Тил',
    'NavTeleprogramBalastan'  =>  'Телеканал Баластан',
    'NavTeleprogramSport'  =>  'Телеканал КТРК Спорт',
    'NavTeleprogramKgRadio'  =>  'Кыргыз радиосу',
    'NavTeleprogramBirinchi'  =>  'Биринчи радио',
    'NavTeleprogramDostuk'  =>  'Радио Достук',
    'NavTeleprogramMinkiyal'  =>  'Радио Миң-Кыял FM',

    'SelectTV'  =>  'Выбрать телеканал',
    'SelectRadio'  =>  'Выбрать радиостанцию',

    # Director Page
    'DirBio' => 'Биография',
    '' => '',
    '' => '',
    '' => '',

    # Post Page
    'PostByDate' => 'Новости по дате',
    'NewsForDate' => 'Новости за',

    # Media
    'Projects' => 'Телепроекты',
    'RelatedMedia' => 'Видео по теме',
    'MediaMaterials' => 'Видеоматериалы',

    # Gallery
    'Galleries' => 'Фоторепортажи',
    '' => '',

    # Search
    'Search' => 'Поиск',
    'DataResult' => 'Результаты по запросу',
    'DataResultTag' => 'Результаты по тегам',
    'DataResultPost' => 'Результаты по названиям новостей',
    'DataResultShows' => 'Результаты по видеоматериалам',
    '' => '',




    // abakan added
    'AdminPostThemeRu' => 'Материалы по теме',
    'Top news' => 'Актуально',
    'All news' => 'Все новости',
    'News' => 'Новости',

    'Ktrk' => 'Общественная телерадиовещательная корпорация',
    'KtrkApp' => 'Мобильные приложения КТРК',
    'Developed' => 'Разработка сайта',
    
    'shortDescriptionKg' => 'Короткое описание на кыргызском',
    'shortDescriptionRu' => 'Короткое описание на русском',

    'Comments' => 'Комментарии',
    'Reply' => 'Комментировать',
    'LeaveComment' => 'Оставить комментарий',
    'CommentSuccess' => 'Ваш комментарий принят тна модерацию!',
    'CommentError' => 'Где-то произошла ошибка!',
    'LoadMore' => 'Загрузить еще',
    'All' => 'Все',

//    'Homepage' => '',
    'NowLive' => 'Сейчас в эфире',
    'AfterLive' => 'Далее в эфире',

    //Header Footer titles
    'Homepage' => 'Главная страница',
    'BaikoochuKenesh' => 'Наблюдательный совет',
    'BaikoochuKeneshFull' => 'Наблюдательный совет Общественной Телерадиовещательной корпорации',

    'BaikoochuKeneshMain' => 'Главная',
    'BaikoochuKeneshTeam' => 'Состав',
    'BaikoochuKeneshNews' => 'Новости',
    'BaikoochuKeneshBase' => 'Нормативная база',
    'BaikoochuKeneshReglament' => 'Регламент',
    'BaikoochuKeneshReports' => 'Отчеты',
    'BaikoochuKeneshGallery' => 'Фоторепортажи',
    'BaikoochuKeneshContacts' => 'Контакты',
    'BaikoochuKeneshContactsFull' => 'Контактная информация',
    'BaikoochuKeneshContactsAdres' => 'Наш адрес',
    'BaikoochuKeneshContactsAdresText' => 'г.Бишкек, бульвар Молодой Гвардии, 59',
    'BaikoochuKeneshContactsPhone' => 'Телефон приемной',
    'BaikoochuKeneshAsk' => 'Опрос',
    'BaikoochuKeneshTeamTitle' => 'Состав Наблюдательного совета Общественной телерадиовещательной корпорации',
    'BaikoochuKeneshReportsTitle' => 'Годовые отчеты о деятельности Наблюдательного Совета ОТРК',
    'ReadMore' => 'Читать далее',
    'MoreNewsBirinchi' => 'Читать далее',
    'ReadLess' => 'Закрыть',


    'RedKenesh' => 'Редакционный советник',
    'RedKeneshNews' => 'Новости',

    'HistoryFull' => 'История Общественной Телерадиовещательной корпорации Кыргызской Республики',
    'History' => 'История',

    'NormalBaseFull' => 'Нормативная база Общественной телерадиовещательной корпорации Кыргызской Республики',
    'NormalBase' => 'Нормативная база',

    'RadioCenterFull' => 'Республиканский радиотелецентр',
    'RadioCenterHistory' => 'История РРТЦ',

    'ReportFull' => 'Отчет о деятельности Общественной телерадиовещательной корпорации Кыргызской Республики за 2010-2016гг.',

    'Strategy' => 'Стратегия развития',

    //TradeMark
    'TradeMark' => 'Общественная Телерадиовещательная корпорация Кыргызской Республики',
    'TradeMarkAll' => 'Все права защищены.',
    'TradeMarkRegulations' => 'Материалы, размещенные на сайте ktrk.kg предназначены только для персонального пользования.',    
    'TradeMarkRegulations2' => 'Использование материалов сайта, копирование, размещение, распространение возможно только при письменном согласовании с администрацией КТРК.',    
    'TradeMarkRegulations3' => 'При согласованном использовании материалов сайта обязательна ссылка на ресурс.',
    'TradeMarkRegulations4' => 'Онлайн-трансляция эфиров КТРК, телеканала “Музыка”, радиостанций КТРК в сети интернет без согласования запрещена.',
    'TradeMarkAddress' => 'Адрес: г.Бишкек, блв.Молодой Гвардии, 59',
    'TradeMarkTel' => 'Тел/факс: +996 312 39 20 59, +996 312 39 24 04',
    'TradeMarkEmail' => 'Эл.почта: public@ktrk.kg',
    'TradeMarkPart' => 'Отдел по поддержке сайта: +996 312 39 22 38',
    'TradeMarkKtrk' => 'Эл.почта: ktrksite@gmail.com',


    'Live' => 'прямой эфир',
    'Telechannels' => 'Телеканалы',
    'RadioStations' => 'Радиостанции',
    'Managers' => 'Руководство',
    'DirectorPage' => 'КТРК - Генеральный директор',
    'More' => 'Подробнее',

//    Reklama
    'Reklama' => 'Рекламно-коммерческий отдел',
    'ReklamaRent' => 'АРЕНДА ТЕХНИКИ',

    //Post page
    'MatpoTeme' => 'Материалы по теме',
    'PostAllNews' => 'Все новости',
    'TekteshNews' => 'Новости по теме',
    '' => '',
    '' => '',
    '' => '',
    '' => '',    

    //One title words
    'Info' => 'Информация',
    'Change' => 'Изменить',
    'Delete' => 'Удалить',
    'AreYouAgree' => 'Вы уверены?',
    'Save' => 'Сохранить',
    'NewArticle' => 'Новая статья',
    'Articles' => 'Статьи',
    'Start' => 'Начало',
    'End' => 'Конец',
    'Tags' => 'Теги',
    'Title' => 'Название',
    'Publish' => 'Опубликовать',
    'PublishStatus' => 'Статус',
    'Back' => 'Назад',
    
    'Top' => 'Вверху',
    'Center' => 'В центре',
    'Left' => 'Налево',
    'Right' => 'Направо',
    'Bottom' => 'Внизу',
    'Content' => 'Содержание',
    'TitleKG' => 'Название на кыргызском',
    'TitleRU' => 'Название на русском',
    'TitleEN' => 'Краткое название на Английском',
    'Home' => 'Главная',

    'TagsKG' => 'Теги на кыргызском',
    'TagsRU' => 'Теги на русском',
    'TextKG' => 'Текст на кыргызском',
    'TextRU' => 'Текст на русском',
    'DescKG' => 'Описание на кыргызском',
    'DescRU' => 'Описание на русском',
    'AuthorKG' => 'Автор на кыргызском',
    'AuthorRU' => 'Автор на русском',
    'Images' => 'Изображения',
    'Select' => 'Выбрать',
    '' => '',

    //    Народный репортер
    'Reporter' => 'Народный репортер',
    'ReporterAdd' => 'Добавить новость',
    'AdminReporterName' => 'Имя',
    'AdminReporter' => '',

    //    Teleprogram
    'Teleprogram' => 'Программа передач',
    'Contacts' => 'Контакты',
    'Ads' => 'Реклама',
    'LiveProgram' => 'Далее в эфире',

    // Admin panel translate
    'AdminHomeTitle' => 'Главная',
    'AdminPhotoUploadTitle' => 'База картинок',
    'AdminChannelTitle' => 'Каналы',
    'AdminPostTitle' => 'Новости',
    'AdminPostsTitle' => 'Новости',
    'AdminPostsCatTitle' => 'Категории новостей',
    'AdminMediaTitle' => 'Медиа новости',
    'AdminMediaPostTitle' => 'Медиа новости',
    'AdminMediaPostCatTitle' => 'Категории медиа новостей',
    'AdminPagesTitle' => 'Страницы',
    'AdminUsersTitle' => 'Пользователи',
    'AdminBackgroundTitle' => 'Фоновый рисунок',
    'AdminaAditionalPages' => 'Дополнительные страницы',
    'AdminHelpSos' => 'Помощь админу',

    //Background changed menu
    'AdminBackgroundNameTitle' => 'Заменить фоновый рисунок',
    'AdminBackgroundNewimg' => 'Новый фоновый рисунок',
    'AdminBackgroundChangedImg' => 'Изменить фоновый рисунок',
    'AdminBackgroundImagesName' => 'Фоновые рисунки',
    'AdminBackgroundSelect' => 'Выберите рисунок',
    '' => '',

    //Banner changed menu
    'AdminBannerCreate' => 'Создать баннер',
    'AdminBannerChanged' => 'Изменить баннер',
    'AdminBannerNew' => 'Новый баннер',
    'AdminBanners' => 'Баннеры',
    'AdminBannerSelectImg' => 'Выбрать рисунок',
    '' => '',

    //Category menu
    'AdminCategoryNew' => 'Новая категория',
    'AdminCategoryChanged' => 'Изменить категорию',
    'AdminCategoryAddNew' => 'Добавить новую категорию',
    'AdminCategories' => 'Категории',
    'AdminMediaCategories' => 'Список телепередач',
    'AdminCategoryTekteshArticles' => 'Все новости (статьи) относяшиеся этой категории',
    'AdminCategoryTitlekg' => 'Тема на кыргызском',
    'AdminCategoryTitleru' => 'Тема на русском',
    '' => '',

    //Channel menu
    'AdminChannelAddNew' => 'Создать новый канал',
    'AdminChannelChange' => 'Изменить канал',
    'AdminChannelNew' => 'Новый канал',
    'AdminChannels' => 'Каналы',
    'AdminChannelAllUser' => 'Все пользователи канала',
    'AdminMediaIMGorVideo' => 'Картинка или Видео',
    '' => '',

    //Media menu
    'AdminMediaAddNewArticle' => 'Создать новую медиа статью',
    'AdminMediaChanged' => 'Изменить медиа статью',
    'AdminMediaNew' => 'Новая медиа статья',
    'AdminMediaArticles' => 'Медиа статьи',
    'AdminMediaTypeSelect' => 'Выберите тип медии',
    'AdminMediaURL' => 'Youtube ID (пример:jh2kL40iY2o)',
    'AdminMediaView' => 'Кол-во просмотров',
    'AdminMediaProject' => 'Передача',
    '' => '',

    //Media Category menu
    'AdminMediaCategoryNew' => 'Создать новую категория для медиа статей',
    'AdminMediaCategoryChange' => 'Изменить медиа категорию',
    'AdminMediaCategoryNewC' => 'Новая категория',
    'AdminMediaCategoryMedias' => 'Медиа категории',
    'AdminMediaCategoryTekteshArticles' => 'Все материалы относящиеся этой категории',
    '' => '',

    //Menu menu
    'AdminMenuAdd' => 'Создать новое меню',
    'AdminMenuChange' => 'Изменить меню',
    'AdminMenuNew' => 'Новая меню',
    'AdminMenuCodeWrite' => 'Написать через код',
    'AdminMenuNewWindow' => 'Новое окно',
    '' => '',

    //Page menu
    'AdminPageAddNew' => 'Создать новую страницу',
    'AdminPageChange' => 'Изменить страницу',
    '' => '',

    //People Reporter menu
    'AdminPeopleReporterNew' => 'Написать новую статью',
    'AdminPeopleReporterChange' => 'Изменить статью',
    '' => '',

    //Photochild menu
    'AdminPhotochildAddNew' => 'Загрузить новую картину',
    'AdminPhotochildImgChange' => 'Изменить картину',
    'AdminPhotochildAddNewImg' => 'Добавить новую картину',
    'AdminPhotochildAllImg' => 'Все картинки',
    'AdminPhotoChildSelectGaller' => 'Выберите фото галерею',
    'DescriptionKg' => 'Описание на кыргызском',
    'DescriptionRu' => 'Описание на русском',

    //PhotoParent menu
    'AdminPhotoParentAddNew' => 'Создать новую фото галерею',
    'AdminPhotoParentChange' => 'Изменить фото галерею',
    'AdminPhotoParentNewGal' => 'Новая фото галерея',
    'AdminPhotoParents' => 'Фото галереи',
    'AdminPhotoParentLocation' => 'Расположение',
    'AdminPhotoParentImages' => 'Добавить изображения',
    'AdminPhotoParentImagesSelect' => 'Выберите изображения',
    '' => '',

    //Post Menu
    'AdminPostAddNew' => 'Создать новую статью',
    'AdminPostChange' => 'Изменить статью',
    'AdminPostTheme' => 'Материал к теме',
    'AdminPostMedia' => 'Медиа к теме',
    'AdminPostThemaone' => 'Материал к теме 1',
    'AdminPostThematwo' => 'Материал к теме 2',
    'AdminPostThemathree' => 'Материал к теме 3',
    'AdminPostEnterText' => 'Введите текст...',
    'AdminPostCreatedDate' => 'Дата создания',
    'AdminPostEdit' => 'Редактировать',

    //User menu
    'AdminUserAddNew' => 'Добавить нового пользователя',
    'AdminUserChange' => 'Изменить данные пользователя',
    'AdminUserNew' => 'Новый пользователь',
    'AdminUserAlls' => 'Все пользователи',
    'AdminUserInfo' => 'Информация о пользователе',
    'AdminUserName' => 'Имя',
    '' => '',

    // Project Menu
    'AdminProjectAll' => 'Передачи',
    'AdminProjectAddNew' => 'Новая передача',
    '' => '',
    '' => '',

    //    Comments
    'AdminCommentChange' => 'Изменить комментарий',

    // Anons
    'AdminAnonsNew' => 'Новый анонс',
    'AdminAnonsWeekDay' => 'День недели на кыргызском',
    'AdminAnonsWeekDayRu' => 'День недели на русском',
    'AdminAnonsTime' => 'Время',

    //Quotes
    'AdminQuoteNew' => 'Новая цитата',
    'AdminQuoteAuthor' => 'Автор на кыргызском',
    'AdminQuoteAuthorRU' => 'Автор на русском',
    'AdminQuoteDesc' => 'Цитата на кыргызском',
    'AdminQuoteDescRU' => 'Цитата на русском',

    'AdminQuoteEdit' => 'Изменить цитату',

    //FRONT language

    //Home page
    'FrontPostAll' => 'Все новости',    
    'FrontPostLastNews' => 'Лента новостей',
    'FrontLastNews' => 'Последние новости',
    'FrontPostDaysVideo' => 'Видео дня',
    'FrontPostDaysNews' => 'Выпуск новостей',
    'FrontPostDaysEpisode' => 'Телепроект',
    'FrontPostDaysAnons' => 'Анонс',
    'FrontPostDaysMaanai' => 'Маанайшат',
    'FrontPostLastDays' => 'Прошедшие дни',
    'FrontDirectorPage' => 'Страница генерального директора',    
    'FrontVideoAll' => 'Перейти на видеопортал',
    'FrontGalleryAll' => 'Все фоторепортажи',
    'FrontToDirectorPage' => 'Перейти на страницу',
    'FrontTeleprogrammChoose' => 'Выберите канал',
    '' => '',
    '' => '',    
    '' => '',

    //Videoportal
    'Monday' => 'Понедельник',
    'Tuesday' => 'Вторник',
    'Wednesday' => 'Среда',
    'Thursday' => 'Четверг',
    'Friday' => 'Пятница',
    'Saturday' => 'Суббота',
    'Sunday' => 'Воскресенье',

    'AllVideos' => 'Все видео',
    'LastVideos' => 'Последние видео',
    'TopVideos' => 'Топ видео',
    'Episodes' => 'Выпуски',
    'CategoryVideos' => 'Все видео из категории',

    // Data Tables
    'DataTableTotal' => 'всего',
    'DataTableSearch' => 'Поиск',
    'DataTableNoResult' => 'Ничего не найдено',
    'DataTableFirstPage' => 'Начало',
    'DataTableLastPage' => 'Конец',
    'DataTableNextPage' => 'След.',
    'DataTablePreviousPage' => 'Пред.',

    //Search page

//    Post
    'PostAktualno'=>'Актуально',
    'PostMain'=>'Актуально',
    'PostPopular'=>'Популярное',

];
