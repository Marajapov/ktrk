<link rel="stylesheet" href="../css/radios.css">

<div style="margin-left: 545px;">
  <div class="box box-widget widget-radio">
    <div class="widget-radio-header">
      <div class="widget-radio-image">
        <img class="img-circle" src="http://beta.ktrk.kg/images/channels/small/6.png" alt="Radio">
      </div>
      <h3 class="widget-radio-name">Биринчи радио</h3>
      <h5 class="widget-radio-desc">Бишкек-104.1, Иссык-Куль-102.4, Нарын-100.5, Ош-100.7, Баткен-104.2, Жалал-Абад-104.7, Талас-102.0</h5>
    </div>
    <div class="radio-body no-padding">
      <div class="onlineone">
        <table border="0" width="311" height="70" cellspacing="0" cellpadding="0">
          <tbody>
<!--          <tr>-->
<!--            <td style="background: black;">-->
<!--              <img style="width: 50px; height: 50px; margin-left: 10px;" src="http://beta.ktrk.kg/images/channels/small/6.png" title="Биринчи радио" alt="Биринчи радио">-->
<!--            </td>-->
            <td style="background: black;">
              <object id="videoplayer367" width="311" height="70" data="http://ktrk.kg/sites/all/themes/sevenmag/js/player/uppod.swf">
                <param name="bgcolor" value="#ffffff" />
                <param name="allowFullScreen" value="true" />
                <param name="allowScriptAccess" value="always" />
                <param name="movie" value="http://ktrk.kg/sites/all/themes/sevenmag/js/player/uppod.swf" />
                <param name="flashvars" value="comment=KTRK&amp;st=03AEEZykN3Q3Q3QH3zkOk1mwXRWNvz3b1XAxYTtj1mzC5kA2uyb1kOk1mwXRWNvhzdwf9vwjzC5k=jQZi6jNf1kOk1mwXRWHNXzBGDY31X=jOkZ2sN6HsQfzC5kev0kttj1vL2NXzC5k1sQE3Q2GkTCGfFdGJTCHwXpvQXeWNhHm0X1hsNkTWwWTSUhTd0kNtj1JTWwf9vwnY31DX1CYLex5W=jaktjR1i6W0kbSal1C0V9Rv1XAuaTtj1tzC5kb42Qi63Gktj1ozfUrd&amp;file=http://212.112.96.232:8080&amp;auto=play" />
              </object>
            </td>
          </tr>
          </tbody>
        </table>
      </div>
    </div>
  </div>
</div>




