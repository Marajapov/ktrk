            ...
            <div class="panel-body">
              <div class="col-md-12 videofix">
                ...
                <nav class="muzpaginate">
                  <ul class="pagination">
                    <li>
                      <a href="{{ route('muzkanal.videos.concert', ['page' => 1]) }}" class="btn btn-default @if($Concert->currentPage() == 1) disabled @endif">{{ trans('site.Start') }}</a>
                    </li>
                    <li>
                    ...
                    @for($i = 0, $j = 1; $i < $Concert->total(); $i+=$perPage)
                      <li>
                        <a href="{{ route('muzkanal.videos.concert', ['page' => $j]) }}" class="btn btn-default @if($Concert->currentPage() == $j) active @endif">{{ $j++ }}</a>
                      </li>
                    @endfor
                    <li>
                      <a href="{{ route('muzkanal.videos.concert', ['page' => ceil($Concert->total()/$perPage)]) }}" class="btn btn-default @if($Concert->currentPage() == ceil($Concert->total()/$perPage)) disabled @endif">{{ trans('site.End') }}</a>
                    </li>
                  </ul>
                </nav>
              </div>
