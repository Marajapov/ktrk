            ...
            <div class="panel-body">
              <div class="col-md-12 videofix">
                ...
                <nav class="muzpaginate">
                  <ul class="pagination">
                    <li>
                      <a href="{{ route('muzkanal.videos.concert', ['page' => 1]) }}" class="btn btn-default @if($MediaLive->currentPage() == 1) disabled @endif">{{ trans('site.Start') }}</a>
                    </li>
                    <li>
                    ...
                    @for($i = 0, $j = 1; $i < $MediaLive->total(); $i+=$perPage)
                      <li>
                        <a href="{{ route('muzkanal.videos.concert', ['page' => $j]) }}" class="btn btn-default @if($MediaLive->currentPage() == $j) active @endif">{{ $j++ }}</a>
                      </li>
                    @endfor
                    <li>
                      <a href="{{ route('muzkanal.videos.concert', ['page' => ceil($MediaLive->total()/$perPage)]) }}" class="btn btn-default @if($MediaLive->currentPage() == ceil($MediaLive->total()/$perPage)) disabled @endif">{{ trans('site.End') }}</a>
                    </li>
                  </ul>
                </nav>
              </div>
