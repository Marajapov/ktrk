...
@section('content')
    ...
    <div class="container">
        <div class="sport-tv clearfix">
            <a class="main" href="{{ route('sport.teleprogram') }}">{{trans('site.LiveProgram')}} <span>{{ date("Y:m:d")}}</span></a>
                    ...
                    @if($program)
                        @foreach($program as $key => $row)
                            @if(($key<(count($program)-1)) && (strtotime($program[$key]->time) <= strtotime($currentTime)) && (strtotime($currentTime) < strtotime($program[$key+1]->time)) )
                                <li class="tele-item live" style="z-index: 99;">
                                ...
                                @if($key == count($program)-2)
                                    <li class="tele-item" style="z-index: 100">
                                ...
                                @else
                                    ...
                                    @for($i=1;$i<$last = ($count-($key+1)) +1;$i++)
                                        <li class="tele-item" style="z-index: {{99-$i}};">
